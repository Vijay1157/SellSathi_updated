...
const ADMIN_PHONE = process.env.ADMIN_PHONE || "+917483743936";
const ALLOW_TEST_UID = IS_DEV; // Only allow test UID in development mode
...
// ============================================
// QUERY LIMITS & CACHING CONFIGURATION
// ============================================
const QUERY_LIMITS = {
    DEFAULT: 100,           // Default limit for queries
    PRODUCTS: 50,          // Products per page
    ORDERS: 50,            // Orders per page
    SELLERS: 50,           // Sellers per page
    REVIEWS: 20,           // Reviews per page
    ADMIN_STATS: 1000      // Max for admin stats (one-time load)
};
...
const CACHE_TTL = {
    ...
    SELLER_INFO: 10 * 60 * 1000,   // 10 minutes
    PRODUCT_INFO: 5 * 60 * 1000,   // 5 minutes
...
// GET /admin/stats - Dashboard statistics (OPTIMIZED)
app.get("/admin/stats", verifyAuth, verifyAdmin, async (req, res) => {
    try {
        ...
        const stats = {
            ...
            pendingApprovals: pendingSellers.data().count,
            ...
            allSellers: totalSellers.data().count
        };

        // Cache for 2 minutes
        setInCache(cacheKey, { success: true, stats }, CACHE_TTL.STATS);
...
// GET /admin/sellers - Pending seller applications (OPTIMIZED)
app.get("/admin/sellers", verifyAuth, verifyAdmin, async (req, res) => {
    try {
        const limit = parseInt(req.query.limit) || QUERY_LIMITS.SELLERS;
        const sellersSnap = await db.collection("sellers")
            .where("sellerStatus", "==", "PENDING")
            .limit(limit)
            .get();

        // Batch fetch user data to avoid N+1 problem
        const sellerIds = sellersSnap.docs.map(doc => doc.id);
        const userPromises = sellerIds.map(id => db.collection("users").doc(id).get());
        const userSnaps = await Promise.all(userPromises);
        
        const userDataMap = {};
        userSnaps.forEach((snap, index) => {
            if (snap.exists) {
                userDataMap[sellerIds[index]] = snap.data();
            }
        });

        const sellers = sellersSnap.docs.map(doc => {
            const sellerData = doc.data();
            const userData = userDataMap[doc.id] || {};
...
// GET /admin/products - All products for review (OPTIMIZED)
app.get("/admin/products", verifyAuth, verifyAdmin, async (req, res) => {
    try {
        const limit = parseInt(req.query.limit) || QUERY_LIMITS.PRODUCTS;
        const page = parseInt(req.query.page) || 1;
        const offset = (page - 1) * limit;

        // Get total count first
        const totalCount = await db.collection("products").count().get();
        
        // Fetch products with pagination
        let query = db.collection("products").limit(limit);
        
        if (offset > 0) {
            // For pagination, we'd need to store lastDoc, but for now use limit/offset approach
            query = db.collection("products").limit(limit);
        }
        
        const productsSnap = await query.get();

        // Batch fetch seller data
        const sellerIds = [...new Set(productsSnap.docs
            .map(doc => doc.data().sellerId)
            .filter(id => id))];
        ...
        const sellerDataMap = {};
        sellerSnaps.forEach((snap, index) => {
            if (snap.exists) {
                sellerDataMap[sellerIds[index]] = snap.data();
            }
        });
        ...
        const products = productsSnap.docs.map(doc => {
            try {
                ...
                const sellerData = sellerDataMap[productData.sellerId] || {};
                const sellerPhone = sellerData.phone || "Unknown";
...
// GET /admin/all-sellers - All sellers regardless of status (OPTIMIZED)
app.get("/admin/all-sellers", verifyAuth, verifyAdmin, async (req, res) => {
    try {
        const limit = parseInt(req.query.limit) || QUERY_LIMITS.SELLERS;
        
        // Check cache
        const cacheKey = getCacheKey('all-sellers', limit);
        const cached = getFromCache(cacheKey);
        if (cached) {
            console.log('[CACHE HIT] All sellers');
            return res.status(200).json(cached);
        }

        const sellersSnap = await db.collection("sellers").limit(limit).get();
        
        // Batch fetch all related data
        const sellerIds = sellersSnap.docs.map(doc => doc.id);
        
        const [userSnaps, productCounts, deliveredOrders] = await Promise.all([
            Promise.all(sellerIds.map(id => db.collection("users").doc(id).get())),
            Promise.all(sellerIds.map(id => 
                db.collection("products").where("sellerId", "==", id).count().get()
            )),
            db.collection("orders")
                .where("status", "==", "Delivered")
                .where("sellerId", "in", sellerIds.slice(0, 10)) // Firestore 'in' limit is 10
                .get()
        ]);
        ...
        productCounts.forEach((countSnap, index) => {
            productCountMap[sellerIds[index]] = countSnap.data().count;
        });
        ...
        deliveredOrders.docs.forEach(doc => {
            const order = doc.data();
            if (order.items && Array.isArray(order.items)) {
                order.items.forEach(item => {
                    const sellerId = item.sellerId;
                    if (sellerId) {
                        if (!revenueMap[sellerId]) {
                            revenueMap[sellerId] = 0;
                            deliveredCountMap[sellerId] = 0;
                        }
                        revenueMap[sellerId] += (item.price || 0) * (item.quantity || 1);
                    }
                });
                // Count unique orders per seller
                const sellerIdsInOrder = [...new Set(order.items.map(item => item.sellerId).filter(id => id))];
                sellerIdsInOrder.forEach(sellerId => {
                    if (deliveredCountMap[sellerId] !== undefined) {
                        deliveredCountMap[sellerId]++;
                    }
                });
            }
        });
        ...
        const sellers = sellersSnap.docs.map(doc => {
            const sellerData = doc.data();
            const userData = userDataMap[doc.id] || {};
            const totalProducts = productCountMap[doc.id] || 0;
            const totalRevenue = revenueMap[doc.id] || 0;
            const deliveredCount = deliveredCountMap[doc.id] || 0;
        ...
        const response = { success: true, sellers };
        setInCache(cacheKey, response, CACHE_TTL.STATS);
...
// GET /admin/orders - All orders (OPTIMIZED)
app.get("/admin/orders", verifyAuth, verifyAdmin, async (req, res) => {
    try {
        const limit = parseInt(req.query.limit) || QUERY_LIMITS.ORDERS;
        const page = parseInt(req.query.page) || 1;
        
        // Check cache
        const cacheKey = getCacheKey('admin-orders', limit, page);
        const cached = getFromCache(cacheKey);
        if (cached) {
            console.log('[CACHE HIT] Admin orders');
            return res.status(200).json(cached);
        }
...
// GET /admin/reviews - Get all reviews for admin dashboard (OPTIMIZED)
app.get("/admin/reviews", verifyAuth, verifyAdmin, async (req, res) => {
    try {
        const limit = parseInt(req.query.limit) || QUERY_LIMITS.REVIEWS;
        
        // Check cache
        const cacheKey = getCacheKey('admin-reviews', limit);
        const cached = getFromCache(cacheKey);
        if (cached) {
            console.log('[CACHE HIT] Admin reviews');
            return res.status(200).json(cached);
        }
        ...
        const productIds = [...new Set(reviewsSnap.docs.map(doc => doc.data().productId).filter(id => id))];
        
        // Batch fetch only needed products
...
app.post("/auth/extract-aadhar", verifyAuth, upload.single("aadharImage"), async (req, res) => {
    try {
        ...
        if (!extractedData.name || !extractedData.aadhaar_no) {
            return res.status(422).json({
                success: false,
                message: 'Could not extract required information from Aadhaar card. Please ensure the image is clear and complete.'
            });
        }

        // Validate Aadhaar number format (12 digits)
        const aadhaarNumber = extractedData.aadhaar_no.replace(/\s/g, '');
        if (!/^\d{12}$/.test(aadhaarNumber)) {
            return res.status(422).json({
                success: false,
                message: 'Invalid Aadhaar number format. Please upload a valid Aadhaar card.'
            });
        }
...
// POST /api/user/:uid/wishlist/add - Add item to wishlist
app.post("/api/user/:uid/wishlist/add", verifyAuth, async (req, res) => {
    try {
        const { uid } = req.params;
        if (req.user.uid !== uid) return res.status(403).json({ success: false, message: "Access denied" });
        ...
        await db.collection("users").doc(uid).collection("wishlist").doc(product.id).set({
            ...product,
...
app.post("/api/user/:uid/address/save", verifyAuth, async (req, res) => {
    try {
        ...
        const requiredFields = ['firstName', 'lastName', 'addressLine', 'city', 'state', 'pincode'];
        ...
        if (!doc.exists) {
            ...
            if (address.id !== undefined && address.id < currentAddresses.length) {
...
app.post("/api/user/:uid/address/delete", verifyAuth, async (req, res) => {
    try {
        ...
        if (doc.exists) {
            ...
            if (addressId >= currentAddresses.length || addressId < 0) {
