...
import { SELLER_CATEGORIES, ALL_SUBCATEGORIES } from '../../config/categories';
import { VARIANT_CONFIGS } from '../../config/productVariants';

// Use the shared category configuration
...
export default function AddProduct() {
    ...
    const [product, setProduct] = useState({
        name: '', price: '', discountPrice: '', category: '', subCategory: '', stock: '', description: '', image: ''
    });
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: '2rem' }}>
                        ...
                        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}
                            ...
                            <div style={sty.card}>
                                ...
                                {/* Subcategory Selector - Shows when category is selected */}
                                {product.category && ALL_SUBCATEGORIES[product.category] && (
                                    <div style={{ marginBottom: '1.5rem' }}>
                                        <label style={sty.label}>Subcategory</label>
                                        <select required style={sty.select} value={product.subCategory}
                                            onChange={e => setProduct({ ...product, subCategory: e.target.value })}>
                                            <option value="">Select Subcategory</option>
                                            {ALL_SUBCATEGORIES[product.category].map(subcat => (
